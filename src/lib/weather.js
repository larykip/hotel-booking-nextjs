...
/**
 * Fetches the current weather data based on the user's location.
...
export async function fetchWeather() {
	try {
		...
		const response = await fetch(`https://api.brightsky.dev/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}&date=${today}`);
		if (!response.ok) {
